...
class ModeloContabilidad
{
    ...
    static public function mdlObtenerContabilidad($tabla)
    {
        $stmt = Conexion::conectar()->prepare("SELECT productos.id, productos.producto, productos.unidad, especialidad, cantidad,  productos.unico, fecha
        FROM $tabla
        INNER JOIN productos ON contabilidad.id_producto = productos.id");
        $stmt->execute();
